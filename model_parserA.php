...
class LogModel
{
    ...
    {
        $sql = "INSERT INTO licenselogA (Date, Time, Software, Status, Feature, UserMachine, Licenses) VALUES (?, ?, ?, ?, ?, ?, ?)";
    ...
    {
        $sql = "SELECT DISTINCT COUNT(Status), Date FROM (SELECT DISTINCT * FROM licenselogA WHERE Status = 'DENIED') AS T GROUP BY Date;";
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute();
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
    ...
    {
        $sql = "SELECT DISTINCT COUNT(Status), Date FROM (SELECT DISTINCT * FROM licenselogA WHERE Status = 'DENIED' AND Date >= ? AND Date <= ?) AS T GROUP BY Date; ";
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute([$startDate, $endDate]);
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
    ...
    public function lic()
    {
        $sql = "SELECT Date, Feature, SUM(Licenses) FROM (SELECT DISTINCT * FROM licenseloga) AS T WHERE Status='OUT' GROUP BY Date,Feature;";
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute();
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
    ...
    {
        $sql = "SELECT Date, Feature, SUM(Licenses) FROM (SELECT DISTINCT * FROM licenseloga WHERE Date >= ? AND Date <= ?) AS T WHERE Status='OUT' GROUP BY Date,Feature;";
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute([$startDate, $endDate]);
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
