...
if(trim($_POST['unit']) == "") 
	{
      $errMsg .= "<li>Please enter your Unit</li>";
    } else { $unit=$_POST['unit']; }
if(trim($_POST['icno']) == "") 
	{
      $errMsg .= "<li>Please enter your ICNO.</li>";
    } else { $icno=$_POST['icno']; $icno = ltrim($icno,"0");}
...
$data[icno]= $icno;//$_POST['icno']; //$data[icno] = ltrim($data[icno],"0");
$data[name]=$_POST['name'];
$data[phone]=$_POST['phone'];
$data[grp]=$_POST['grp'];
$data[subgrp]=$_POST['subgrp'];
$data[email]=$_POST['email']; 
$data[lognames]=$_POST['lognames'];
$data[ipadd]=$_POST['ipadd'];
$data[des]=$_POST['des'];
$data[sec]=$_POST['sec'];
$data[div]=$_POST['div'];
$data[clusacct]=$_POST['clusacct'];
$data[homefldr]=$_POST['homefldr'];
$data[homefldr_existing]=$_POST['homefldr_existing'];
$data[adacct]=$_POST['adacct'];
$data[app]=$_POST['others'];
$data[serapp]=$_POST['serapp'];
if((empty($data[serapp])) && (empty($data[app])) )
{
$errMsg .= 'Please Select Any of the Application software / Provide software Name in Others<br>';
}
//if((!empty($_POST['unit']))&&(!empty($_POST['icno'])) &&(!empty($_POST['name']))&&(!empty($_POST['phone']))&& (!empty($_POST['email'])) && (!empty($_POST['grp'])))
if($errMsg=='')
{  
   //echo "----------------coming inside if";
   if (!preg_match("/^[a-zA-Z. ]*$/",$data[name])) 
     {
   $errMsg .= "Only letters, white space and dot allowed for name <br>"; 
     }
...
if($errMsg=='')
{
$reqid=$this->usermodel->user_req($data);
include 'webmail.php';
$d = new Zmail;
...
echo "<script> window.location.assign('index.php?hpcpage=hpcuserprt');</script> "; 
} 
//if
} //post if 


include "view/hpcuserreq.php";
} //controller function

// Processig WS request
public function wsuser()
{
if(isset($_POST['cfetch'])) 
{
	$reqid=$_POST['wsreqid'];
	$r1=$this->usermodel->fetchrequest($reqid);
}
if(isset($_POST['csave'])) 
{
$errMsg ='';
...
{
	$au=$this->usermodel->fetchadminusers();
	//print_r($au);
...
if(isset($_POST['divforward']))
{
	...
	if((!empty($_POST['check_list'])) && (!empty($_POST['uname']))){
echo "*************";
$auser=$_POST['uname'];
foreach($_POST['check_list'] as $selected){
...
public function hpcadminprocess()
{
	echo "*************";
$r1=$this->usermodel->fetchadminapproval();
print_r($r1);
include "view/hpcadminapproval.php";

}

public function addhpcuser()
{
...
$uicno=$_GET[ICNO];
echo $uicno;
//$r1=$this->usermodel->searchldap($uicno);	


include "view/addhpcuser.php";

}


public function ldapusersall()
{
	$ldaprdn="cn=Manager,dc=igcar,dc=cd";
	$ldappass="manager123";
	$ldapconn=ldap_connect("10.20.2.19");##Changed for testing secondary ldap

//$this->usermodel->ldapconnect();
...
public function wsldapusersall()
{
	
	/*
	//$ldaprdn="cn=gws.gov.in,dc=GWS,dc=GWS";
	$ldappass="gwsdomain@2019";
	$ldapconn=ldap_connect("10.29.3.62");
