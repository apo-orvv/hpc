...
class SmDetailsController{
	...
	$sysparams_user_role=$this->smdetailsmodel->getsystemparamsfordisplay($systemname,'U');
        if($systemparam==''){
...
//	include "view/displayjobs.php";
		...
		if(strcmp($systemparam,"Backup_Status")==0){
					
		$bkstat=$this->smdetailsmodel->getBackupStat($systemname);
				include "view/displayBackupstat.php";
		} 
		...
		if(strcmp($systemparam,"GWS_History_Usage")==0) {
				...
				$WSnameall=$this->smdetailsmodel->getWSNames();
				
				$msg="    ** For Graph Display, Please Choose GWS Name, From Date and To Date  **    ";
		if(isset($_POST['showgwsusage']) ){
				...
				if ((!($fromval)) || (!($toval)))
				{
					$msg="    ** Please Choose From Date and To Date **    ";
				
				}
				else if (($fromval) > ($toval))
				{
					$msg="    ** Please Choose From Date Less than To Date **    ";
				
				}
				else { list($graphlabels,$WSnameall,$WSReport)=$this->smdetailsmodel->getWSReportGraph($wsname,$fromval,$toval);  }
		...
		if(strcmp($systemparam,"GWS_Monthly_Usage")==0) {
			
							
		if(isset($_POST['showgwsusage']) ){
        		if(isset($_POST['monyear'])){
        		 $monyear = filter_input(INPUT_POST,'monyear',FILTER_SANITIZE_FULL_SPECIAL_CHARS); 
				 
				list($mon,$year)=split(" ",$monyear);
				if ($mon=="January") $mon1=1;
				if ($mon=="February") $mon1=2;
				if ($mon=="March") $mon1=3;
				if ($mon=="April") $mon1=4;
				if ($mon=="May") $mon1=5;
				if ($mon=="June") $mon1=6;
				if ($mon=="July") $mon1=7;
				if ($mon=="August") $mon1=8;
				if ($mon=="September") $mon1=9;
				if ($mon=="October") $mon1=10;
				if ($mon=="November") $mon1=11;
				if ($mon=="December") $mon1=12;
        		}		
			if ((!($monyear)) )
				{
			...
			list($WSnameall,$WSReport)=$this->smdetailsmodel->getWSReport("All",$mon1,$year); 	
			$pagetitle="Workstation Usage for $mon $year";
			}
		}	
			else {		
		list($WSnameall,$WSReport)=$this->smdetailsmodel->getWSReport("All","CM","CM"); 
						
			$pagetitle="Workstation Current Month Usage";	}		
	...
	if(strcmp($systemparam,"Utilization_Month")==0) {
		
		
				//$swnameall=$this->smdetailsmodel->getSWNames();
								
				$msg="    ** For Graph Display, Please Choose Software Name, From Date and To Date  **    ";
		if(isset($_POST['showswusage']) ){
								$chartmsg="Click on the Software modules to select / deselect";	
			if(isset($_POST['summarytype'])){ 
			$usagetype = filter_input(INPUT_POST,'summarytype',FILTER_SANITIZE_FULL_SPECIAL_CHARS);
			
			}
				
        		if(isset($_POST['partition'])){
        		 $swname = filter_input(INPUT_POST,'partition',FILTER_SANITIZE_FULL_SPECIAL_CHARS); 
				}
        		if(isset($_POST['fromdate'])){
        		 $fromval = filter_input(INPUT_POST,'fromdate',FILTER_SANITIZE_FULL_SPECIAL_CHARS); 
		...
		$UsedFeature=$this->smdetailsmodel->getFeaturename($swname,$fromval,$toval);
		
		$subtitle="$swname usage from $fromval to $toval";
		
		$xaxis="Date";
		$i=0;
		foreach($UsedFeature as $msg1=>$val)
		{
			$usedfea=$val;
			$SWReport[]=$this->smdetailsmodel->getSWReportGraph($swname,$usedfea,$fromval,$toval);
		}
		...
		//print_r($UsedFeature);
		$subtitle="$swname usage from $fromdate to $todate";
		$xaxis="Date";
		$i=0;
		foreach($UsedFeature as $msg1=>$val)
		{
			$usedfea=$val;
			$SWReport[]=$this->smdetailsmodel->getSWReportGraphPer($swname,$usedfea,$fromval,$toval);
		...
		include "view/displaylicenseusage.php";
...
if(strcmp($systemparam,"ANSYS_19")==0){
		...
		if(strcmp($systemparam,"RECURDYN")==0){
		$software="RECURDYN";
		$licenses=$this->smdetailsmodel->getlicensestatus($software);
		include "view/displaylicensestatus.php";
		}
		...
		if(strcmp($systemparam,"License_Users")==0){
			...
			$swname1 = filter_input(INPUT_POST,'swname',FILTER_SANITIZE_FULL_SPECIAL_CHARS);
			
			$licenseusers=$this->smdetailsmodel->getlicenseuserstatus($swname1);
			}				
		...
		//Licenses History Usage
		
		if(strcmp($systemparam,"Utilization_Day")==0) { 
			
		include "view/displaysystembanner.php";
		
		include "view/softwareusage";
		
		if(isset($_POST['showswusage']) ){ 
		
		$swname = filter_input(INPUT_POST,'showswusage',FILTER_SANITIZE_FULL_SPECIAL_CHARS);
		
		if ($swname=="Abaqus") {
		include "view/displayAbaqususage.php";}
		
		if ($swname=="Ansys_17") {
		include "view/displayAnsysusage17.php";}
		
		if ($swname=="Ansys_18") {
		include "view/displayAnsysusage18.php";}
				
		if ($swname=="Ansys_21") {
		include "view/displayAnsysusage.php";}
                
                if ($swname=="Comsol") {
		include "view/displayComsolusage.php";}
		
		if ($swname=="RECURDYN") {
		include "view/displayRecurdynusage.php";}
                
		if ($swname=="CFD_SYSWELD") {
		include "view/displayCFDusage.php";}
