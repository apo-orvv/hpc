...
class SmDetailsModel{
	...
	$systemparamsadmin=array('Ivy'=>['Nodes','Nodes (Detailed)','Queues','Jobs','JobsChart','Users','Cluster_Usage_History','Queue_Usage','Storage_Logs','Storage_Status','Storage_Servers','Job_Accounting','User_Details','DDN_Storage','Backup_Status'],
				...
				'Licenses'=>['License_Users','ANSYS_17_1','ANSYS_18_2','ANSYS_19','Abaqus','CFD-ACE/SYSWELD','COMSOL','RECURDYN','Utilization_Month','Utilization_Day'],
		...
		$systemparamsuser=array('Ivy'=>['Nodes','Nodes (Detailed)','Queues','Jobs','JobsChart','Users','User_Details'],
				...
				'Licenses'=>['License_Users','ANSYS_17_1','ANSYS_18_2','ANSYS_19','Abaqus','CFD-ACE/SYSWELD','COMSOL','RECURDYN']);
...
//For IGcsc		$req=$dbcon->prepare("SELECT UsedPercent as UsedPercent,timeofmon as MYHOUR FROM `clusterusage` where DATE(timeofmon)= :today and clustername='$systemname'  ");
	...
	public function getlicenseuserstatus($software){
		...
		foreach ($req->fetchAll() as $msg){
                        $licenseusers[$i]=array($msg['username'],$msg['licenseserver'],$msg['feature'],$msg['machinename'],$msg['starttime_formatted'],$msg['software'],$msg['licenses']);
                        $i++;
...
public function getlicenseusedstatus(){
		...
		$req1=$dbcon->prepare("SELECT max(hpcs_timestamp) as TimeofMon from softwarelicenses ");
		$req1->execute();
	...
	/*Converting Time Format For temporary Purpose*/
	
	
	
public function getFeatureusage($swname,$fname){
...
//echo $fname;
		...
		$this_day=date("Y-m-d");
		$reqfea=$dbcon->prepare("SELECT feature,HOUR(timeofmon) as MYHOUR,total,used FROM softwarelicenseshistory where software='$swname' and date(timeofmon)='$this_day' and feature='$fname' group by hour(timeofmon)");
		$reqfea->execute(array('swname' => $swname,'fname' => $fname,'today' => $this_day));
	
		foreach ($reqfea->fetchAll() as $msg){
			
			//echo "Coming to Model";
			$feausage[]=array($msg['feature'],$msg['MYHOUR'],$msg['total'],$msg['used']);
...
$req1=$dbcon->prepare("SELECT DISTINCT gwsname from gwsusage");
	...
	public function getADusers(){
		...
		foreach ($req->fetchAll() as $msg){
           ...
           $gwsadusers[$i]=array($msg['gwsaccname'],$msg['gwsusername'],$usergroup[1],$msg['gwslastlogin'],$msg['emailid'],$msg['EmpType'],$msg['EmpNumber'],$msg['Empou'],$msg['EmpTel']);
                        $i++;
	...
	public function getlicensestatus($software){
		...
		foreach ($req->fetchAll() as $msg){
                        $licenses[$i]=array($msg['hpcs_timestamp'],$msg['licenseserver'],$msg['feature'],$msg['total'],$msg['used'],$msg['timeofmon']);
                        $i++;
